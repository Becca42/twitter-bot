...
def binary_search_tweets_by_date(tweets, targetDate, start, end):
    ...
    if (start >= end):
        ...
        print("target: "+ str(targetDate))
        print("closest?: " + str(tweets[start-1].created_at) + "\n")
        return start - 1
...
if __name__ == '__main__':
    ...
    #WOWOWOW DEBUG CRAP
    # equality = (allTweets[1] == sorted(allTweets[1], key=lambda x: x.created_at))
    # reverseEquality = (allTweets[1] == sorted(allTweets[1], key=lambda x: x.created_at, reverse=True))

    # CONFIRMED THAT CONTEXT TWEETS ARE LISTED IN ORDER OF DATE_CREATED, MOST TO LEAST RECENT (DESCENDING ORDER)

    print("1 newest: " + str(allTweets[1][0].created_at) + "\n")
    print("1 oldest: " + str(allTweets[1][-1].created_at) + "\n")

    print("numContext1: " + str(len(allTweets[1])) + "\n")
    print("numUser: " + str(len(allTweets[0])))

    # I THINK THE PROBLEM IS THAT SNOPES TWEETS WAY LESS FREQUENTLY THAN THESE OTHER PLACES
    # => HOW TO SOLVE?? REPLACE SNOPES AS TARGET? MAYBE...
